...
from pDynamoWrapper.PESAnalysis import PESAnalysis
...
class EnergyAnalysis:
    ...
    def Path_From_PES(self, in_point,fin_point,_path_pkl,_folder_dst,_system,min_points=17,max_points=21):
        ...
        pathx = []
        pathy = []
        PES_analysis = PESAnalysis( self.energiesMatrix, in_point, fin_point ,self.xlen, self.ylen )
        PES_analysis.Analysis()
        for i in range(1,len(PES_analysis.pathx)): 
            pathx.append(PES_analysis.pathx[i])
            pathy.append(PES_analysis.pathy[i])
        ...
        kcats = [0.0]
        new_idx = 0
        min_energy = self.energiesMatrix[ pathy[0] ][ pathx[0] ]
        
        self.energies1D = []
        for idx in range( len(pathx) ):
            self.energies1D.append( self.energiesMatrix[ pathy[idx] ][ pathx[idx] ] )
        ...
        for indx in range( 1,len(pathx) ):            
            ...
            new_idx +=1
            if self.energiesMatrix[pathy[indx]][pathx[indx]] < min_energy:
                min_energy = self.energiesMatrix[pathy[indx]][pathx[indx]]
    ...
    def ResamplePath(self, path_indices,min_points=17, max_points=21, include_curvature=True):
        ...
        # Always keep the original first and last
            ...
            candidates = set(range(1, last_minima)) - keep
            if candidates:
