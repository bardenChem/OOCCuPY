...
def Run_Test():
	...
	if not os.path.exists( os.path.join(test_01_folder, "7tim_pruned_and_fix.pkl") ):
		try: subprocess.run([sys.executable, str(Path(__file__).with_name("test_01_system_setup.py"))], check=True)
		except: 
	...
	if not os.path.exists( os.path.join(test_18_folder, "7tim_qcmm_rm1_pruned.pkl") ):
		try: subprocess.run([sys.executable, str(Path(__file__).with_name("test_18_qmmm_setup.py"))], check=True)
		except: 
